...
def LoadDataset():
    path = '/media/songguoxian/b1efdde7-81bb-4db1-897b-b5ff506288bc/songguoxian/Dataset/'
    with open(path+'datase0.pickle', 'rb') as f:
        save = pickle.load(f)
        part_real_label = save['part_real_label']
        part_real_img = save['part_real_img']
        part_synthetic_label =save['part_synthetic_label']
        part_synthetic_img =save['part_synthetic_img']
    return part_real_label,part_real_img,part_synthetic_label,part_synthetic_img

class DataLoader(object):
  def __init__(self, config, rng=None):
    self.part_real_label, self.part_real_img, self.part_synthetic_label, self.part_synthetic_img = LoadDataset()
    ...
    self.real_data = np.reshape(self.part_real_img,[np.shape(self.part_real_img)[0],52,72])

    synthetic_path ='/media/songguoxian/b1efdde7-81bb-4db1-897b-b5ff506288bc/songguoxian/Dataset/Neutal/Synthesis/Selected/'
    synthetic_data = []
    for i in range(50410):
      synthetic_data.append(synthetic_path+str(i)+'.png')
    self.synthetic_data_paths = np.array(synthetic_data)
